...
/**
 * SkyColorSystem
 * Procedural sky dome with day/night modes, animated clouds, moon phases.
 * Can composite a custom HDRI texture behind the cloud layer.
 */
export class SkyColorSystem {
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    if (dtm > moonSize) {
                        ...
                        vec3 moonLocalRight = normalize(cross(moonDir, vec3(0.0, 1.0, 0.0)));
                        ...
                        if (phaseInvert < phaseCutoff || u_moonPhase > 0.95) {
                            nightBackground = mix(nightBackground, vec3(0.95, 0.95, 0.85), smoothstep(0.997, 1.0, dtm));
                        }
    ...
    toggle(forceState, currentTexture) {
        ...
        if (this.visible) {
            // When sky override is ON: use sky dome as background, hide HDRI background
            this.setBackgroundTexture(currentTexture);
            this.scene.background = null; // Sky dome takes over visually
        } else {
            // When sky override is OFF: restore HDRI as scene background
            if (currentTexture) {
