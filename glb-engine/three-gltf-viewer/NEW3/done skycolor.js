...
/**
 * SkyColorSystem — Procedural sky dome
 *
 * ROOT CAUSE FIX for black sky:
 *   - scene.background = null clears to renderer clearColor (black).
 *   - The dome sphere should cover the whole view, but on some GPU drivers
 *     the BackSide sphere has rendering order issues.
 *   - FIX: When sky override is ON, we set scene.background to a matching
 *     solid sky-blue Color. The dome renders on top and covers everything.
 *     If the dome somehow has a gap, you see blue instead of black.
 *   - Also: frustumCulled = false so the sphere is ALWAYS rendered even
 *     if the camera is inside it (which it is), and scale it up to 900.
 */
export class SkyColorSystem {
    constructor(scene) {
        ...
        // The "horizon color" — used as scene.background fallback when dome is active
        // so if dome has any gaps, you see blue not black
        this._horizonHex = '#7dd3fc';

        this._init();
    }

    _init() {
        // Use a large sphere — camera is inside it
        const geo = new THREE.SphereGeometry(900, 32, 15);

        this.mat = new THREE.ShaderMaterial({
            side: THREE.BackSide,
            depthWrite: false,
            depthTest: false,        // render regardless of depth buffer
            uniforms: {
            ...
            vertexShader: `
                ...
                void main() {
                    ...
                    vWorldPos = wp.xyz;
                    // Force to far plane so it renders behind everything
                ...
                void main() {
                    ...
                    if (dtm > moonSize) {
                        ...
                        if (phase < cut || u_moonPhase > 0.95) {
                            nightBg = mix(nightBg, vec3(0.95,0.95,0.85), smoothstep(0.997, 1.0, dtm));
                        }
        ...
        this.mesh.renderOrder    = -2;       // render before anything else
        this.mesh.frustumCulled  = false;    // CRITICAL: camera is inside sphere
    ...
    /**
     * Returns a THREE.Color matching the current horizon (for scene.background fallback).
     * Viewer sets scene.background to this so black never shows through any gaps.
     */
    getHorizonColor() {
        if (this.params.nightMode > 0.5) {
            return new THREE.Color(0x020208); // near-black for night
        }
        return new THREE.Color(this._horizonHex);
    }
    ...
    setNightMode(on) {
        this.mat.uniforms.u_nightMode.value = on ? 1.0 : 0.0;
        this.params.nightMode = on ? 1.0 : 0.0;
    }
